...
$qr= "SELECT * FROM `codehistory` WHERE `codeID`=$id ORDER BY `date_history`;";
$query = mysqli_query( $conn, $qr);
//echo $qr;
...
<table class="table table-hover" style="color: #f05f40">
  ...
  <tr style="background-color: #333333;">
    <th>CodeID</th><th>Code Link</th><th>Date</th><th> &nbsp; &nbsp;&nbsp; &nbsp;  View</th><th>&nbsp; &nbsp; &nbsp; &nbsp; Delete</th>
  </tr>
  <?php while ($row = mysqli_fetch_array ($query)) { ?>
  <tr><td><?php echo  $row['codeID']; ?></td> <!--while hobe-->
    <td><?php echo $row['userCodeLink']; ?></td>
    <td><?php echo $row['date_history'];  ?></td>
 <td><form method="post" action="showHistory.php?id=<?php echo $row['orderID']; ?>"><button style="width: 60%; font-size: 10pt;" class="btn btn-ls btn-primary btn-block" name="submit" type="submit">VIEW</button></form></td>
<td><form method="post" action="deleteHistory.php?id=<?php echo $row['orderID']; ?>"><button style="width: 60%; font-size: 10pt;" class="btn btn-ls btn-primary btn-block" name="submit" type="submit">Delete</button></form></td> 
  </tr>
  <?php } ?>

</table>
<br/>
<a href="code.php" class="btn btn-primary btn-xm page-scroll" style= "Margin: 0 0 0 80%; width: 20%;"> BACK</a>
