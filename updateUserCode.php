...
//echo "$id";
  ...
  $q="SELECT * FROM `usercodes` WHERE `codeID`=$id;";
  $qee_q = mysqli_query( $conn, $q);
  $row = mysqli_fetch_array ($qee_q);

  $previousLocationWouldbe =  $row['userCodeLink'];
  $currcode= $previousLocationWouldbe;
  
  //echo "$previousLocationWouldbe";
  $newName= "$previousLocationWouldbe.t";
  while(file_exists("$newName")) {
     $previousLocationWouldbe= $newName;
     $newName="$previousLocationWouldbe.t";
  }
  //echo "$currcode \n";
  //echo "$newName";
  copy($currcode,$newName );
  $date= date("Y-m-d") . "<br>";
 // echo $date;
$userName= $_SESSION['username'];
  $q="INSERT INTO `codehistory` (`orderID`, `codeID`, `userName`, `userCodeLink`, `date_history`) VALUES (NULL, '$id', '$userName', '$newName', '$date');";
 //echo $q;
 $qee_q = mysqli_query( $conn, $q);
 //echo $id;
      
?>
 <form  method="post" enctype="multipart/form-data" action ="updateuserCodephp.php?id=<?php echo $id; ?>">
