...
?>
<table class="table table-hover" style="color: #aaaaaa">
  

  
  <tr style="background-color: #333333;">
    <th>CodeID</th><th>CodeTitle</th><th>CodeName</th><th>Status</th><th>Privecy</th><th>View</th>
  </tr>
...
?></td>
  <td><?php $userAcc= $row['userAccess'];if($userAcc == 0) {echo "PRIVATE"; } else {echo "PUBLIC";} ?></td>
...
<div style="Margin: 10%" >
          <h2 class="section-heading" style="text-align: center; color: #f05f40;  font-style: bold;"> Your Codes History!</h2>
                    <hr class="primary">
                  </div>
...
$qrrr=  "SELECT * FROM `codehistory`  WHERE `userName`='$username' ORDER BY `codehistory`.`date_history` DESC";
//echo $qrrr;
$queryy = mysqli_query( $conn, $qrrr);
...
<table class="table table-hover" style="color: #aaaaaa">
  ...
  <tr style="background-color: #333333;">
    <th>CodeID</th><th>OrderID</th><th>userCodeLink</th><th>Date</th><th>VIEW</th>
  </tr>
  <?php while ($row = mysqli_fetch_array ($queryy)) { ?>
  
  <tr><td><?php echo $row['codeID']; ?></td>
  <td><?php echo $row['orderID']; ?></td>
  <td><?php echo $row['userCodeLink']; ?></td>
  <td><?php echo $row['date_history']; ?></td>
 <td><form method="post" action="showHistory.php?id=<?php echo $row['orderID']; ?>"><button style="width: 60%; font-size: 10pt;" class="btn btn-lm btn-primary btn-block" name="submit" type="submit">View</button></form></td>
